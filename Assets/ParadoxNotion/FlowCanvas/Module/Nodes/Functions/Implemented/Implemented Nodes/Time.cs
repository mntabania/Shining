...
namespace FlowCanvas.Nodes
{
	...
	public class DampFloat : PureFunctionNode<float, float, float>{
		private float last;
		public override float Invoke(float value, float damp = 1f){
			last = Mathf.Lerp(last, value, damp * Time.deltaTime);
			return last;
		}
	}
	...
	[Category("Time")]
	[Description("Returns a smoothly interpolated value towards the input value.")]
	public class DampVector3 : PureFunctionNode<Vector3, Vector3, float>{
		private Vector3 last;
		public override Vector3 Invoke(Vector3 value, float damp = 1f){
			last = Vector3.Lerp(last, value, damp * Time.deltaTime);
			return last;
		}
	}
